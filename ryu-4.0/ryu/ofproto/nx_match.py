...
class ClsRule(ofproto_parser.StringifyMixin):
    ...
    def set_tp_dst(self, tp_dst):
        LOG.debug('VLOG in point tp_dst') #CEP
    ...
    def set_e_attr1_masked(self, e_attr1, mask):
        LOG.debug('VLOG in point 61') #CEP
        self.wc.e_attr1_mask = mask
        self.flow.e_attr1 = e_attr1 & mask    

    def set_e_attr2(self, e_attr2):
        LOG.debug('VLOG in point 71') #CEP
        self.set_e_attr2_masked(e_attr2, UINT64_MAX)

    def set_e_attr2_masked(self, e_attr2, mask):
    ...
    def set_e_type(self, e_type):
        ...
        self.set_e_type_masked(e_type, UINT64_MAX)

    def set_e_type_masked(self, e_type, mask):
        LOG.debug('VLOG in point 171') #CEP
        self.wc.e_type_mask = mask
        self.flow.e_type = e_type & mask 
    ...
    def match_tuple(self):
        ...
        if self.flow.e_attr1 != 0:
            ...
            wildcards &= ~ofproto_v1_0.OFPFW_EVNT_ATTR1 

        if self.flow.e_attr2 != 0:
        ...
        return (wildcards, self.flow.in_port, self.flow.dl_src,
                ...
                self.flow.e_attr2, self.flow.e_type, self.flow.e_val1)
...
class MFField(object):
    ...
    def putm(self, buf, offset, value, mask):
        LOG.DEBUG('VLOG in MFFIELD putm') #CEP
        if mask == 0:
            return 0
...
class MFEVNTATTR1(MFField):
    @classmethod
    def make(cls, header):
        LOG.debug('VLOG in point 73')
        return cls(header, MF_PACK_STRING_BE64)

    def put(self, buf, offset, rule):
        LOG.debug('VLOG in point 83')
        return self.putm(buf, offset, rule.flow.e_attr1, rule.wc.e_attr1_mask)


@_register_make
@_set_nxm_headers([ofproto_v1_0.NXM_OF_EVNT_ATTR2])
class MFEVNTATTR2(MFField):
    @classmethod
    def make(cls, header):
        LOG.debug('VLOG in point 93')
        return cls(header, MF_PACK_STRING_BE64)
...
def serialize_nxm_match(rule, buf, offset):
    ...
    if rule.flow.e_attr1 != 0:
        ...
        if rule.flow.nw_proto == 17 and rule.wc.e_attr1_mask == UINT64_MAX:
            header = ofproto_v1_0.NXM_OF_EVNT_ATTR1
        else:
            header = 0
        if header != 0:
            offset += nxm_put(buf, offset, header, rule)                

    if rule.flow.e_attr2 != 0:
        LOG.debug('VLOG in point 112')
        if rule.flow.nw_proto == 17 and rule.wc.e_attr2_mask == UINT64_MAX:
            header = ofproto_v1_0.NXM_OF_EVNT_ATTR1
        else:
            header = 0
        if header != 0: 
