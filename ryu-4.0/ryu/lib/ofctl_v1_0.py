...
def to_match(dp, attrs):
    ...
    LOG.debug('VLOG In to_match OFPFW_EVNT_ATTR1 %d', ofp.OFPFW_EVNT_ATTR1) #CEP
    LOG.debug('VLOG In to_match OFPFW_EVNT_ATTR2 %d', ofp.OFPFW_EVNT_ATTR2) #CEP
    in_port = 0
    dl_src = 0
    dl_dst = 0
    dl_vlan = 0
    ...
    for key, value in attrs.items():
        if key == 'in_port':
            ...
            LOG.debug('VLOG In to_match point -13  OFPFW_EVNT_ATTR1 - %d wildcards - %d', ofp.OFPFW_EVNT_ATTR1, wildcards) #CEP
        elif key == 'e_attr2':
            e_attr2 = int(value)
